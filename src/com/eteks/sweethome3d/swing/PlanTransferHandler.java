...
public class PlanTransferHandler extends LocatedTransferHandler
{
	...
	protected Transferable createTransferable(final JComponent source)
	{
		...
		if (source instanceof PlanComponent)
		{
			...
			return new Transferable()
			{
				...
				{
					return transferable.isDataFlavorSupported(flavor) || DataFlavor.imageFlavor.equals(flavor);
				}
			};
		}
		else
	...
	{
		if (transferable.isDataFlavorSupported(HomeTransferableList.HOME_FLAVOR) && destination instanceof PlanComponent
				&& this.homeController.getPlanController() != null)
		{
			try
			{
				...
				this.homeController.getPlanController().startDraggedItems(transferedItems, (float) dropLocation.getX(),
						(float) dropLocation.getY());
	...
	protected void dragMoved(JComponent destination, Transferable transferable, int dragAction)
	{
		...
		{
			Point2D dropLocation = getDropModelLocation(destination);
			this.homeController.getPlanController().moveMouse((float) dropLocation.getX(), (float) dropLocation.getY());
		}
	}
	...
	private boolean importHomeTransferableList(final JComponent destination, final List<Selectable> transferedItems)
	{
		if (isDrop())
		{
			Point2D dropLocation = getDropModelLocation(destination);
			...
			{
				this.homeController.drop(transferedItems, (float) dropLocation.getX(), (float) dropLocation.getY());
			}
		}
		else
		{
	...
	{
		final Point2D dropLocation = isDrop() ? getDropModelLocation(destination) : new Point2D.Float();
		final List<String> importableModels = getModelContents(files, contentManager);
		EventQueue.invokeLater(new Runnable()
		{
			public void run()
			{
				homeController.dropFiles(importableModels, (float) dropLocation.getX(), (float) dropLocation.getY());
			}
		});
