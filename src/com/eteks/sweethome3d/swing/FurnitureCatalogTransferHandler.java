...
public class FurnitureCatalogTransferHandler extends VisualTransferHandler
{
	 ...
	 */
	public FurnitureCatalogTransferHandler(ContentManager contentManager, FurnitureCatalogController catalogController,
		...
		try
		{
			if (transferable.isDataFlavorSupported(HomeTransferableList.HOME_FLAVOR))
			{
				...
				if (transferedItems.size() == 1)
				{
					...
					{
						return IconManager.getInstance().getIcon(((PieceOfFurniture) transferedItem).getIcon(), 48,
								null);
					}
				}
			}
	...
	protected Transferable createTransferable(JComponent source)
	{
		...
		{
			List<HomePieceOfFurniture> transferedFurniture = new ArrayList<HomePieceOfFurniture>(
					selectedCatalogFurniture.size());
	...
	{
		return this.catalogController != null && Arrays.asList(flavors).contains(DataFlavor.javaFileListFlavor);
	}
