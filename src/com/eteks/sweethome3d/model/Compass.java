...
public class Compass implements Serializable, Selectable
{
	...
	{
		X, Y, DIAMETER, VISIBLE, NORTH_DIRECTION, LATITUDE, LONGITUDE, TIME_ZONE
	}
	...
	public void setNorthDirection(float northDirection)
	{
		if (northDirection != this.northDirection)
		{
			...
			this.pointsCache = null;
			this.propertyChangeSupport.firePropertyChange(Property.NORTH_DIRECTION.name(), oldNorthDirection,
					northDirection);
		}
	}
	...
	public float[][] getPoints()
	{
		if (this.pointsCache == null)
		{
			...
			Rectangle2D pieceRectangle = new Rectangle2D.Float(getX() - getDiameter() / 2, getY() - getDiameter() / 2,
		...
		return new float[][] { this.pointsCache[0].clone(), this.pointsCache[1].clone(), this.pointsCache[2].clone(),
				this.pointsCache[3].clone() };
	}
	...
	public boolean intersectsRectangle(float x0, float y0, float x1, float y1)
	{
		...
		rectangle.add(x1, y1);
		return new Ellipse2D.Float(getX() - getDiameter() / 2, getY() - getDiameter() / 2, getDiameter(), getDiameter())
	...
	{
		Ellipse2D shape = new Ellipse2D.Float(getX() - getDiameter() / 2, getY() - getDiameter() / 2, getDiameter(),
				getDiameter());
		if (margin == 0)
		{
	...
	{
		if (this.dateCache == null || this.dateCache.getTimeInMillis() != date)
		{
			...
			double rightAscension = Math
					.toDegrees(Math.atan(
							Math.cos(Math.toRadians(e)) * Math.sin(Math.toRadians(l)) / Math.cos(Math.toRadians(l))))
					/ 15.;
			...
			double elevation = Math.asin(Math.sin(declination) * Math.sin(this.latitude)
					- Math.cos(declination) * Math.cos(this.latitude) * Math.cos(hourAngle));
			double azimuth = Math.acos((Math.sin(declination) - Math.sin(this.latitude) * Math.sin(elevation))
					/ (Math.cos(this.latitude) * Math.cos(elevation)));
	...
	private double computeJulianDay(int year, int month, int day, int hour, int minute, int second, int timeZone,
			int savingTime)
	{
	...
	private double toSiderealTime(double julianDay)
	{
		...
		double siderealTime = (24110.54841 + (8640184.812866 * centuries) + (0.093104 * Math.pow(centuries, 2.))
				- (0.0000062 * Math.pow(centuries, 3.))) / 3600.;
	...
	private void initGeographicPoint()
	{
		...
		if (timeZoneGeographicPointsReference == null || timeZoneGeographicPointsReference.get() == null)
		{
			...
			timeZoneGeographicPoints.put("America/Indiana/Indianapolis",
					new GeographicPoint(39.7683333f, -86.1580556f));
			...
			timeZoneGeographicPoints.put("America/Argentina/Catamarca",
					new GeographicPoint(-28.4666667f, -65.7833333f));
			...
			timeZoneGeographicPoints.put("Iceland", new GeographicPoint(64.1333f, -21.9333f)); // Reykjav�k
			...
			timeZoneGeographicPointsReference = new WeakReference<Map<String, GeographicPoint>>(
